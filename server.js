...
app.post('/api/reportar', async (req, res) => {
    ...
    const telefonoFinal = [telefono, whatsapp, celular, tel]
    .find(t => t && t.trim() !== '')?.trim() || "S/D";
...
app.post('/api/buscar', async (req, res) => { 
    try {
        ...
        const yaExisteBusquedaEnAdmin = await Busqueda.findOne({
    nro: nroLimpio,
    categoria: catFija
});

        // BUSCAMOS SI YA FUE ENCONTRADO (MATCH)
        const yaEncontrado = await Hallazgo.findOne({
    nro: nroLimpio,
    categoria: catFija
});
...
app.get('/api/puntos-venta', async (req, res) => {
    try {
        const locales = await PDV.find(); // Trae todos los locales de la base de datos
        res.json(locales);
    } catch (error) {
